...
# Use conda to access Python which is needed for clustering, umap and other python packages
...
# Dataset description
* Analysis of DE differentiation data
...
# Project ID
param$project_id = "N1660_N1690"

# Input data path
param$path_data = data.frame(name=c("dm3", "dm1_SD", "dm1_CA", "d0_SD", "d0_CA", "d3_SD", "d3_SD_2D", "d3_CA"),
                             type=c("10x","10x","10x","10x","10x","10x","10x","10x"),
                             path=c("/mnt/ngsnfs/single_cell_dev/example_data/N1660_N1683-N1667_N1690/02_DataProcessing_cellranger/02_counts/N1663_N1686/outs/filtered_feature_bc_matrix", "/mnt/ngsnfs/single_cell_dev/example_data/N1660_N1683-N1667_N1690/02_DataProcessing_cellranger/02_counts/N1661_N1684/outs/filtered_feature_bc_matrix", "/mnt/ngsnfs/single_cell_dev/example_data/N1660_N1683-N1667_N1690/02_DataProcessing_cellranger/02_counts/N1662_N1685/outs/filtered_feature_bc_matrix", "/mnt/ngsnfs/single_cell_dev/example_data/N1660_N1683-N1667_N1690/02_DataProcessing_cellranger/02_counts/N1666_N1689/outs/filtered_feature_bc_matrix", "/mnt/ngsnfs/single_cell_dev/example_data/N1660_N1683-N1667_N1690/02_DataProcessing_cellranger/02_counts/N1667_N1690/outs/filtered_feature_bc_matrix", "/mnt/ngsnfs/single_cell_dev/example_data/N1660_N1683-N1667_N1690/02_DataProcessing_cellranger/02_counts/N1664_N1687/outs/filtered_feature_bc_matrix", "/mnt/ngsnfs/single_cell_dev/example_data/N1660_N1683-N1667_N1690/02_DataProcessing_cellranger/02_counts/N1660_N1683/outs/filtered_feature_bc_matrix",
"/mnt/ngsnfs/single_cell_dev/example_data/N1660_N1683-N1667_N1690/02_DataProcessing_cellranger/02_counts/N1665_N1688/outs/filtered_feature_bc_matrix"),  
                             stats=c(NA,NA,NA,NA,NA,NA,NA,NA))

# Output directory
param$path_out = "/mnt/ngsnfs/single_cell_dev/users/kosankem/scrnaseq_v1.2/results/"
...
param$file_known_markers = "/mnt/ngsnfs/single_cell_dev/users/kosankem/scrnaseq_v1.2/Target_genes.xlsx"
...
# Filters
param$cell_filter = list(nFeature_RNA=c(2200, 9000), percent_mt=c(NA, 18))
param$feature_filter = list(min_counts=1, min_cells=3, min.features = 200) # feature has to be found by at least one count in one cell
...
param$vars_to_regress = c("percent_mt", "nCount_RNA")
...
The following table shows metadata (columns) of the first 5 cells (rows). These metadata provide additional information about the cells in the dataset, such as the sample a cell belongs to ("orig.ident"), the above mentioned number of unique genes detected ("nFeature"), or the percentage of mitochondrial genes ("percent_mt"). 
...
knitr::kable(purrr::map(sc[i], function(s) {head(s[[]])}), align="l", caption=names(sc[i])) %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>%
...
```{r highest_expression, eval=FALSE}
#Text
...
#  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"))
```
nFeature_RNA `r cell_filter_tbl[1,1]` - `r cell_filter_tbl[1,2]` and percent_mt `r cell_filter_tbl[2,1]` - `r cell_filter_tbl[2,2]`.
...
QC metric for cells after filtering
```{r qc_plot_correlation_after_filter}
# Combine cell metadata of the Seurat objects into one big metadata
sc_cell_metadata_v2 = suppressWarnings(purrr::map_dfr(sc, function(s){ s[[]] }) %>% as.data.frame())

# Correlate QC metrics for cells
p_list = list()
p_list[[1]] = ggplot(sc_cell_metadata_v2, aes_string(x=cell_qc_features[2], y=cell_qc_features[1], colour="orig.ident")) +
  geom_point() + 
  AddStyle(col=param$col_samples)
p_list[[2]] = ggplot(sc_cell_metadata_v2, aes_string(x=cell_qc_features[3], y=cell_qc_features[1], colour="orig.ident")) +
  geom_point() + 
  AddStyle(col=param$col_samples)
p = patchwork::wrap_plots(p_list, ncol=2) + patchwork::plot_annotation("Features plotted against each other after filtering")
...
p_list = purrr::map(list_names(sc), function(n) {
  ...
  p = Seurat::VariableFeaturePlot(sc[[n]], 
                                  assay=param$norm, 
                                  selection.method=ifelse(param$norm=="RNA", "vst"), 
...
### Variable genes
```{r Variable_genes_merged}
# Show variable genes
if (param$norm == "RNA") { 
top10 = head(Seurat::VariableFeatures(sc, assay="RNA"), 10)
p = Seurat::VariableFeaturePlot(sc, assay="RNA", selection.method="vst", col=c("grey", param$col)) + 
    theme(legend.position=c(0.2, 0.8), legend.background=element_rect(fill=alpha("white", 0.0)))
p = LabelPoints(plot=p, points=top10, repel=TRUE, xnudge=0, ynudge=0)
p
}
...
p = Seurat::ElbowPlot(sc, ndims=20) + 
  geom_vline(xintercept=param$pc_n, col="firebrick", lty=2) + 
...
p = Seurat::DimPlot(sc, reduction="umap", label=TRUE) + 
  AddStyle(title=paste0("UMAP, cells coloured by cluster identity; intermediate resolution (", param$cluster_resolution, ")"), legend_position="bottom", legend_title="Clusters") + 
  scale_color_manual(values=param$col_clusters, labels=cluster_labels)
p
```

### Low and high resolution clustering as comparison
While we used an intermediate resolution of `r param$cluster_resolution` for the clustering (displayed above) as standard for the analysis, different resolutions would lead to diverse numbers of clusters. Below we display an example for a low and high resolution clustering. 
```{r}
# Low and high resolution as comparison
sc_low_r = Seurat::FindClusters(sc_low_r, resolution=0.3, algorithm=4, verbose=FALSE, method="igraph")

sc_low_r = BuildClusterTree(sc_low_r, dims=1:param$pc_n, verbose=FALSE)

sc_low_r = suppressWarnings(Seurat::RunUMAP(sc_low_r, dims=1:param$pc_n, verbose=FALSE))

p = Seurat::DimPlot(sc_low_r, reduction="umap", label=TRUE) + 
  AddStyle(title="UMAP, cells coloured by cluster identity; low resolution (0.3)", legend_position="bottom", legend_title="Clusters")
p


sc_high_r = Seurat::FindClusters(sc_high_r, resolution=0.8, algorithm=4, verbose=FALSE, method="igraph")

sc_high_r = BuildClusterTree(sc_high_r, dims=1:param$pc_n, verbose=FALSE)

sc_high_r = suppressWarnings(Seurat::RunUMAP(sc_high_r, dims=1:param$pc_n, verbose=FALSE, umap.method="uwot"))

p = Seurat::DimPlot(sc_high_r, reduction="umap", label=TRUE) + 
  AddStyle(title="UMAP, cells coloured by cluster identity; high resolution (0.8)", legend_position="bottom", legend_title="Clusters")
p
```
...
if ((nrow(degs) > 0) & (nrow(degs_filt$all) > 0)) {
  ...
  p = DegsPlotNumbers(degs_up = degs_filt$down, degs_down = degs_filt$up, 
...
```{r degs_plot_violin_raw, eval=FALSE, fig.height=fig_height_degs}
#### Violin plots (raw)
if (degs_found) {
...
if (degs_found) {
  ...
  p_list = Seurat::VlnPlot(sc, features=genes_example, combine=FALSE, pt.size=0)
  for (i in seq(p_list)) p_list[[i]] = p_list[[i]] + AddStyle(title=genes_example_titles[i], 
                                                              legend_title="Cluster", 
                                                              fill=param$col_clusters, 
                                                              xlab="Cluster") 
...
```{r degs_plot_ridge_raw, eval=FALSE, fig.height=fig_height_degs, message=FALSE}
#### Ridge plots (raw)
if (degs_found) {
...
top5 = degs_top  %>% 
      dplyr::select(gene) %>% 
      as.vector()
p = Seurat::DotPlot(sc, features=top5, cols=c("lightgrey", param$col)) + 
        AddStyle(title=paste0("Top markers (up-regulated genes) for all clusters"), ylab="Cluster", legend_position="bottom") + 
        theme(axis.text.x = element_text(angle=90, hjust=1, vjust=.5)) + 
