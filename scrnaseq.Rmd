...
param$cluster_resolution_test=c(0.3, 0.7, 0.9)
...
The clustering was performed based on `r param$pc_n` principal components. In the subsequent analysis the resolution `r param$cluster_resolution` was used, but other clustering by using different resolutions would be possible.

```{r clustering_part1, warning=FALSE, fig.height=5, fig.width=17}
...
# Test and print different resolutions defined in param$cluster_resolution_test
...
p = Seurat::DimPlot(sc, reduction="umap", pt.size=param$pt_size, label=TRUE, label.size = 5 ) + 
  AddStyle(title=paste0("Cluster resolution: ",i), legend_position="bottom") +
  FontSize(x.text = 14, y.text = 14, x.title = 18, y.title = 18, main = 22) +
...
p = patchwork::wrap_plots(p_list) + patchwork::plot_annotation(title="UMAP, cells coloured by cluster identity;", theme = theme(plot.title = element_text(size = 22)) )  
p
...
cluster_cells = table(sc@active.ident)
cluster_labels = paste0(levels(sc@active.ident)," (", cluster_cells[levels(sc@active.ident)],")")
p = Seurat::DimPlot(sc, reduction="umap", label=TRUE, pt.size=param$pt_size) + 
...
p = Seurat::DimPlot(sc, reduction="umap", pt.size=param$pt_size, label=TRUE, split.by = "orig.ident", ncol = 2) + 
  AddStyle(title="UMAP, cells coloured by cluster identity", legend_position="bottom", legend_title="Clusters") + 
  scale_color_manual(values=param$col_clusters, labels=cluster_labels)
p
```
...
p = Seurat::DimPlot(sc, group.by="orig.ident", pt.size=param$pt_size, cols=param$col_samples) +
...
# Plot all samples separately
p = Seurat::DimPlot(sc, group.by="orig.ident", pt.size=param$pt_size, cols=param$col_samples, split.by = "orig.ident", ncol = 2) +
  AddStyle(title="UMAP, cells coloured by sample of origin", legend_position="bottom")
p$data$seurat_clusters = sc[["seurat_clusters"]][rownames(p$data), ]
p = LabelClusters(p, id="seurat_clusters")
p
...
p = Seurat::DimPlot(sc, group.by="Phase", pt.size=param$pt_size, cols=Misc(sc, "colour_lists")[["Phase"]]) + 
...
p = Seurat::FeaturePlot(sc, features="S.Score", pt.size=param$pt_size, min.cutoff="q1", max.cutoff="q99", label=TRUE, cols=c("lightgrey", param$col)) + 
...
p = Seurat::FeaturePlot(sc, features="G2M.Score", pt.size=param$pt_size, min.cutoff="q1", max.cutoff="q99", cols=c("lightgrey", param$col), label=TRUE) + 
...
p = Seurat::FeaturePlot(sc, features="CC.Difference", pt.size=param$pt_size, min.cutoff="q1", max.cutoff="q99", cols=c("lightgrey", param$col), label=TRUE) +
...
p1 = suppressMessages(Seurat::FeaturePlot(sc, features="nCount_RNA", pt.size=param$pt_size, label=TRUE) + 
...
p1 = suppressMessages(Seurat::FeaturePlot(sc, features="nFeature_RNA", pt.size=param$pt_size, label=TRUE) + 
  AddStyle(title="Feature plot") + 
  scale_colour_gradient(low="lightgrey", high=param$col, trans="log10"))
...
p1 = Seurat::FeaturePlot(sc, features="percent_mt", cols=c("lightgrey", param$col), pt.size=param$pt_size, label=TRUE) + 
  AddStyle(title="Feature plot")
...
if ((known_markers_n > 0) & length(known_markers_vect) <= 100) {
  p_list = Seurat::FeaturePlot(sc, features=known_markers_vect, cols=c("lightgrey", param$col), pt.size=param$pt_size, combine=FALSE, label=TRUE)
  for (i in seq(p_list)) p_list[[i]] = p_list[[i]] + AddStyle()
...
if (markers_found) {
  ...
  for (i in 1:nrow(markers_top)) { 
    p_list[[i]] = Seurat::FeaturePlot(sc, features=markers_top$gene[i], 
                                      cols=c("lightgrey", param$col_clusters[markers_top$cluster[i]]), pt.size=param$pt_size, 
